  ...
  Properties
  {
    ...
    SubShader
    {
      ...
      Pass
      {
        ...
        float GazePos_X[1024];
        float GazePos_Y[1024];
        ...
        fixed4 frag(v2f i) : SV_Target
        {
          ...
          for (float i = 0.0; i < _HitCount; i++)
          {
            ...
            float2 GazePos = float2(GazePos_X[i], GazePos_Y[i]);
            
            PointWeight += 0.5 * distsq(uv,GazePos) * Intensity * _Strength * (1 + sin(_Time.y * _PulseSpeed));

            //totalWeight += 0.5 * distsq(uv, work_pt) * pt_intensity * _Strength * (1 + sin(_Time.y * _PulseSpeed));
          }
          return col + float4(getHeatForPixel(PointWeight), .5);
        }
